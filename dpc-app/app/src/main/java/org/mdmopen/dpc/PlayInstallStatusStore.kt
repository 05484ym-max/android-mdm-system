...
/** Persisted UI state so a process/activity restart never invents progress. */
object PlayInstallStatusStore {
    ...
    fun snapshot(context: Context): PlayInstallUiSnapshot? {
        ...
        val stage = runCatching {
            PlayInstallStage.valueOf(p.getString(KEY_STAGE, PlayInstallStage.PREPARING.name)!!)
        }.getOrDefault(PlayInstallStage.FAILED)
        return PlayInstallUiSnapshot(
            sessionId = sessionId,
