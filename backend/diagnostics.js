...
function diagnose(device, now = Date.now()) {
  ...
  if (device.isDeviceOwner === false) {
    faults.push({
      ...
      description: 'המכשיר דיווח שהוא אינו רשום כ-Device Owner. רוב יכולות הניהול (מדיניות, נעילת קיוסק, חסימת הסרת התקנה) תלויות בהרשאה הזו ואינן פעילות ללא Device Owner.',
  ...
  if (device.lastUpdateStatus === 'FAILED' && !updateFailureResolved(device)) {
    faults.push({
      ...
      recommendedAction: 'נסה עדכון מחדש באמצעות הכפתור כאן. אם הניסיון נכשל שוב, בדוק את פרטי השגיאה, החיבור לרשת והשטח הפנוי במכשיר.',
  ...
  if (seenAge != null && seenAge <= DEFAULT_WARNING_AFTER_MS) {
    ...
    if (syncAge > LAST_SYNC_STALE_AFTER_MS) {
      faults.push({
        ...
        recommendedAction: 'נסה סנכרון מחדש באמצעות הכפתור כאן. לאחר מכן רענן את האבחון כדי לוודא שהמכשיר השלים סנכרון בפועל.',
        remoteFixAvailable: true,
        physicalAccessRequired: false,
