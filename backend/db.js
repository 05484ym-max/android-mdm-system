...
-- Media/file/link attachments for one customer_updates row. A separate
-- table (rather than a JSONB column on customer_updates) because an update
-- can carry several attachments and each one needs its own identity to be
-- individually deletable/streamable by id. ON DELETE CASCADE means deleting
-- an update always takes its attachment rows with it at the database level -
-- the GitHub Release assets backing IMAGE/VIDEO/FILE rows still need
-- explicit cleanup though, since Postgres cascade can't reach outside the
-- database (see deleteCustomerUpdateAtomic).
-- storage_key/file_url/filename/mime_type/size_bytes are only ever set for
-- an IMAGE/VIDEO/FILE row (the uploaded object); link_url/link_label are
-- only ever set for a LINK row (no upload at all) - enforced by the CHECK
-- below rather than trusted to callers.
CREATE TABLE IF NOT EXISTS customer_update_attachments (
  id          UUID PRIMARY KEY,
  update_id   UUID NOT NULL REFERENCES customer_updates(id) ON DELETE CASCADE,
  ...
  link_label  TEXT,
  CHECK (
    (kind IN ('IMAGE', 'VIDEO', 'FILE') AND storage_key IS NOT NULL AND file_url IS NOT NULL AND link_url IS NULL)
    OR
    (kind = 'LINK' AND link_url IS NOT NULL AND storage_key IS NULL AND file_url IS NULL)
  )
);

CREATE INDEX IF NOT EXISTS customer_update_attachments_update_idx
  ON customer_update_attachments (update_id, sort_order, created_at);
`;
...
function mapAttachmentRow(row) {
  ...
  // LINK rows carry an external url + optional label and never a stored
  // file; IMAGE/VIDEO/FILE rows carry `url` pointing at our own streaming
  // route (see GET /api/customer-updates/attachments/:id/download) plus
  // the uploaded file's own metadata. Same field name (`url`) either way,
  // so a caller never has to branch on kind just to know where to fetch
  // this attachment from.
  if (row.kind === 'LINK') {
    return { ...base, url: row.link_url, label: row.link_label };
  }
  return {
    ...base,
    url: row.file_url,
...
/**
 * Batch-fetches attachments for many updates in one query (used by both
 * listCustomerUpdatesForAdmin and listPublishedCustomerUpdatesForDevice) -
 * avoids an N+1 query per update. Returns a Map<updateId, attachment[]>,
 * each list already in sort_order/created_at order; an id with no
 * attachments simply has no entry - callers default to [].
 */
async function listAttachmentsForUpdateIds(updateIds) {
  const map = new Map();
  if (!updateIds.length) return map;
  const { rows } = await pool.query(
    `SELECT * FROM customer_update_attachments
      WHERE update_id = ANY($1::uuid[])
      ORDER BY sort_order ASC, created_at ASC`,
    [updateIds],
  );
  for (const row of rows) {
    if (!map.has(row.update_id)) map.set(row.update_id, []);
    map.get(row.update_id).push(mapAttachmentRow(row));
  }
  return map;
}

async function countAttachmentsForUpdate(updateId) {
  const { rows } = await pool.query(
    `SELECT COUNT(*)::int AS count FROM customer_update_attachments WHERE update_id = $1`,
    [updateId],
  );
  return rows[0].count;
}
...
/**
 * Records one already-uploaded IMAGE/VIDEO/FILE attachment. This function
 * only ever writes metadata - the caller uploads to GitHub Releases first
 * (see apkStorage.uploadAttachment) and passes back the resulting
 * storageKey/fileUrl, same "storage write happens before the DB write, and
 * the caller cleans up storage if this fails" split already used by
 * insertUploadedApp for the app catalog. `id` is generated by the caller
 * (crypto.randomUUID()).
 */
async function addCustomerUpdateFileAttachment(id, updateId, {
  kind, sortOrder, storageKey, fileUrl, filename, mimeType, sizeBytes,
}) {
  const { rows } = await pool.query(
    `INSERT INTO customer_update_attachments
       (id, update_id, kind, sort_order, storage_key, file_url, filename, mime_type, size_bytes)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
     RETURNING *`,
    [id, updateId, kind, sortOrder, storageKey, fileUrl, filename, mimeType, sizeBytes],
  );
  return mapAttachmentRow(rows[0]);
}

/** Records one external-link attachment - no upload/storage involved at all. */
async function addCustomerUpdateLinkAttachment(id, updateId, { sortOrder, url, label }) {
  const { rows } = await pool.query(
    `INSERT INTO customer_update_attachments
       (id, update_id, kind, sort_order, link_url, link_label)
     VALUES ($1, $2, 'LINK', $3, $4, $5)
     RETURNING *`,
    [id, updateId, sortOrder, url, label || null],
  );
  return mapAttachmentRow(rows[0]);
}

/**
 * Unlike mapAttachmentRow's public shape (which the admin list and device
 * feed both use, and which deliberately never exposes the internal GitHub
 * storage_key - same "storage key is admin-server-only" convention as
 * apps_catalog's apkStorageKey), this also returns `storageKey` directly.
 * It exists only for the two internal callers that actually need it: the
 * attachment download route (to fetch the right GitHub asset) and
 * deleteCustomerUpdateAttachment below (to clean it up) - neither one ever
 * echoes storageKey back into an HTTP response.
 */
async function getCustomerUpdateAttachmentById(id) {
  const { rows } = await pool.query(
    `SELECT * FROM customer_update_attachments WHERE id = $1`,
    [id],
  );
  if (!rows[0]) return null;
  return { ...mapAttachmentRow(rows[0]), storageKey: rows[0].storage_key };
}

/**
 * Deletes one attachment row and hands back its metadata (storage_key in
 * particular) so the caller can best-effort clean up the GitHub Release
 * asset afterward. A single DELETE...RETURNING is enough here (unlike
 * deleteCustomerUpdateAtomic below) - one row in one table, no cross-table
 * side effects to coordinate. `updateId` must match the row's own
 * update_id, so a URL naming both ids (see index.js) can never delete an
 * attachment that doesn't actually belong to the update it claims to.
 * Returns null (caller 404s) if no such attachment/update pair exists.
 */
async function deleteCustomerUpdateAttachment(id, updateId) {
  const { rows } = await pool.query(
    `DELETE FROM customer_update_attachments WHERE id = $1 AND update_id = $2 RETURNING *`,
    [id, updateId],
  );
  if (!rows[0]) return null;
  // Includes storageKey (see getCustomerUpdateAttachmentById's comment) so
  // the caller can clean up the GitHub Release asset for a file-backed row.
  return { ...mapAttachmentRow(rows[0]), storageKey: rows[0].storage_key };
}
...
async function listCustomerUpdatesForAdmin() {
  ...
  const attachments = await listAttachmentsForUpdateIds(rows.map(row => row.id));
  return rows.map(row => ({
    ...mapCustomerUpdateRow(row),
...
async function getCustomerUpdateById(id) {
  ...
  if (!rows[0]) return null;
  const attachments = await listAttachmentsForUpdateIds([id]);
  return { ...mapCustomerUpdateRow(rows[0]), attachments: attachments.get(id) || [] };
}
...
/**
 * Atomically deletes a customer_updates row together with its attachments:
 * reads every attachment's storage_key (for GitHub Release cleanup) and
 * deletes the update row - which cascades the attachment rows via ON
 * DELETE CASCADE - inside one transaction, so a crash partway through can
 * never leave the update gone but its attachment rows/GitHub assets
 * orphaned, or vice versa. Same pattern and reasoning as
 * deleteAppFromCatalogAtomic elsewhere in this file.
 *
 * Returns null if no such update existed (caller 404s, nothing touched).
 * On success, returns the list of storage_keys (file-backed attachments
 * only - LINK rows contribute none) the caller should best-effort delete
 * from GitHub AFTER this commits - that cleanup is deliberately not part
 * of this transaction, since a storage failure must never undo an
 * already-committed delete.
 */
async function deleteCustomerUpdateAtomic(id) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    const { rows: updateRows } = await client.query(
      `SELECT id FROM customer_updates WHERE id = $1 FOR UPDATE`,
      [id],
    );
    ...
    const { rows: attachmentRows } = await client.query(
      `SELECT storage_key FROM customer_update_attachments
        WHERE update_id = $1 AND storage_key IS NOT NULL`,
      [id],
    );

    await client.query(`DELETE FROM customer_updates WHERE id = $1`, [id]);

    await client.query('COMMIT');
    return attachmentRows.map(row => row.storage_key);
  } catch (e) {
    await client.query('ROLLBACK');
    throw e;
  } finally {
...
module.exports = {
  ...
  deleteCustomerUpdateAtomic,
  listPublishedCustomerUpdatesForDevice,
  countAttachmentsForUpdate,
  addCustomerUpdateFileAttachment,
  addCustomerUpdateLinkAttachment,
  getCustomerUpdateAttachmentById,
  deleteCustomerUpdateAttachment,
};
