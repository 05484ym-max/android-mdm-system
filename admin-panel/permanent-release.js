(function(){
  ...
  async function releaseDevice(deviceId,button){
    ...
    try{
      const res=await fetch(`/api/devices/${encodeURIComponent(deviceId)}/commands`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({command:'RELEASE_DEVICE_OWNER',params:{},adminPassword})});
  ...
  function mountDetail(){const detail=document.getElementById('detailContent');if(!detail||!detail.children.length||detail.querySelector('[data-permanent-release-card]'))return;const id=window.__currentDetailDeviceId||null;if(!id)return;const h=document.createElement('div');h.innerHTML=`<div class="detail-section detail-section-danger"><h3>אזור מסוכן</h3>${cardHtml(id,false)}</div>`;const section=h.firstElementChild;detail.appendChild(section);bind(section.querySelector('[data-permanent-release-card]'));}
